...
// START MODIFICATION: Add new function for additional sections
function drawAdditionalSections(
  state: PageState,
  additionalSections: any[] = [],
  PDF_CONFIG: any
): number {
  const validSections = Array.isArray(additionalSections)
    ? additionalSections.filter(
        (section) =>
          isValidField(section?.title) &&
          Array.isArray(section?.bullets) &&
          section.bullets.some((bullet: string) => isValidField(bullet))
      )
    : [];
...
    const validBullets = section.bullets.filter((b: string) => isValidField(b));
...
      state.currentY += PDF_CONFIG.spacing.bulletListSpacing;
    });
    state.currentY += 2; // Add space after the section's bullets
  });
...
      // START MODIFICATION: Call drawAdditionalSections for experienced user
      drawAdditionalSections(state, (resumeData as any).additionalSections, PDF_CONFIG);
      // END MODIFICATION
...
      // START MODIFICATION: Call drawAdditionalSections for student
      drawAdditionalSections(state, (resumeData as any).additionalSections, PDF_CONFIG);
      // END MODIFICATION
    } else {
      // Fresher
...
      drawCertifications(state, resumeData.certifications as any[], PDF_CONFIG);
      drawAchievementsAndExtras(state, resumeData, PDF_CONFIG);
      // START MODIFICATION: Call drawAdditionalSections for fresher
      drawAdditionalSections(state, (resumeData as any).additionalSections, PDF_CONFIG);
      // END MODIFICATION
    }
...
      : '';
  
  // START MODIFICATION: Generate HTML for additional sections
  const additionalSectionsHtml =
    (data as any).additionalSections && (data as any).additionalSections.length > 0
      ? (data as any).additionalSections
          .map((section: any) => {
            if (
              !isValidField(section.title) ||
              !Array.isArray(section.bullets) ||
              section.bullets.filter((b: string) => isValidField(b)).length === 0
            ) {
              return '';
            }

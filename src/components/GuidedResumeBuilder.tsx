...
const ResumeOptimizer: React.FC<ResumeOptimizerProps> = ({
  ...
  const [isResumeGenerated, setIsResumeGenerated] = useState(false); // Added: State to track if resume is generated

  // Define the steps array
  const steps = [
    { id: 'profile', title: 'Your Profile', isValid: true },
    { id: 'contact-info', title: 'Contact Info', isValid: true },
    { id: 'objective', title: 'Objective', isValid: true },
    { id: 'education', title: 'Education', isValid: true },
    { id: 'work-experience', title: 'Work Experience', isValid: true },
    { id: 'projects', title: 'Projects', isValid: true },
    { id: 'skills', title: 'Skills', isValid: true },
    { id: 'certifications', title: 'Certifications', isValid: true },
    { id: 'additional-sections', title: 'Additional Sections', isValid: true },
    { id: 'review-generate', title: 'Review & Generate', isValid: isResumeGenerated }, // Validity depends on resume generation
    { id: 'final-resume', title: 'Final Resume', isValid: true },
  ];
  ...
  const handleStartNewResume = useCallback(() => { // Memoize
    ...
    setIsResumeGenerated(false); // Reset generation status
  }, []);
  ...
  const proceedWithFinalOptimization = useCallback(async (resumeData: ResumeData, initialScore: DetailedScore, accessToken: string) => { // Memoize
    try {
      ...
      const beforeScoreData = generateBeforeScore(reconstructResumeText(finalOptimizedResume)); // Use finalOptimizedResume for beforeScore
  ...
  const handleOptimize = useCallback(async () => { // Memoize
    ...
    setOptimizationInterrupted(false);
    setIsResumeGenerated(false); // Reset generation status at the start of a new optimization attempt
    ...
    try {
      ...
      try {
        ...
        if (parsedResumeData && checkForMissingSections(parsedResumeData).length === 0) {
          ...
          const parsedResume = await optimizeResume(
           extractionResult.text,
            ...
            undefined,
            targetRole,
            undefined, // additionalSections parameter
            'deepseek/deepseek-r1:free' // Add this line to specify the model
          );
        ...
        if (missing.length > 0) {
          ...
          setIsOptimizing(false);
          setIsResumeGenerated(false); // Set to false if missing sections
          return;
        }
